...
export function SiteFooterView({ config }: { config: SiteFooterConfig }) {
  const year = new Date().getFullYear();
  const logoSrc = config.settings.logoSrc?.trim() || KEYRA_LOGO_SRC;

  return (
    <footer id="site-footer" className="keyra-site-footer">
        ...
        <div className="keyra-site-footer__grid">
          <section className="keyra-site-footer__block keyra-site-footer__block--brand">
            <span className="inline-flex items-center gap-2.5">
              <span className="relative block h-8 shrink-0 sm:h-9" style={{ aspectRatio: "1320/520" }}>
                <Image
                  src={logoSrc}
                  alt="Keyra"
                  fill
